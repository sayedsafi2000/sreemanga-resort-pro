      ...
      <Container className="relative z-10">
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {items.map((t, i) => (
            <blockquote
              key={t.id}
              className={`flex h-full flex-col rounded-[1.85rem] border border-forest-100 bg-white/95 p-6 shadow-soft ring-1 ring-forest-50 transition hover:-translate-y-1 ${
            ...
            >
              ...
              <p className="mt-4 flex-1 leading-relaxed text-stone-700">&ldquo;{t.quote}&rdquo;</p>
              <footer className="mt-6 text-sm font-semibold text-forest-900">
                {t.author}
                {t.role && (
                  <span className="block font-normal text-stone-500">{t.role}</span>
                )}
              </footer>
            </blockquote>
          ))}
